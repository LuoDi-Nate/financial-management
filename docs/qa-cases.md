...
| v11916-SILENT-BALANCE-REWRITE | **改一笔收入会静默改掉已校准的余额**(2026-09-06 · 复现 issue #17 时顺带坐实)。删一笔 1,000 + 加一笔 4,321,已关账期的期末余额合计从 7,497,726.96 变成 7,501,047.96 —— 页面没有任何提示。提交者把这描述成「已校准的余额失效」,**实际比这更重:不是失效,是被改了**。他因此把上个月账期重新打开重填了一遍。另外顺带查清了他困惑的第二个来源:**本月还没填余额的账户,它的变动不进 ΔNW** —— 复现时删掉的那 1,000 在「招行储蓄卡-工资」上,该账户当月未填余额(19 个账户填了 18 个),于是这 1,000 对仪表盘毫无影响。「改了数据但数字不动」因此**确实可能发生**,而页面没解释这个条件。两条都进 v1.20。 |
