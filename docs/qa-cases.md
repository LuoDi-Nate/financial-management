...
| v11916-CACHE-WITH-NO-INVALIDATION | **一张从来没有人失效过的缓存:数字全更新了,只有 AI 解读没动**(2026-09-06 · v1.19.16 · 线上 issue #17)。用户报「改完上月、重新关账后仪表盘还是之前的数据」。**用真浏览器照他的路径复现**(重开 2026-08 → 删一笔 1000 收入 → 重录一笔 4321 → 重新关账):仪表盘 ΔNW 从 ¥-122,101 变成 ¥-126,422,差额与改动金额完全一致 —— **数字是更新的**。真正没更新的是「AI 月度复盘」:`review_ai_cache` 按 `(family_id, period_id, dim)` 存,而 `PeriodService.reopen()` 只清了 `period_completion` 和 `period_account_attr`,**全仓没有任何地方失效过这张表**(`ReviewAiCacheMapper` 里连 delete 方法都没有)。实测坐实:period 162 那条缓存的时间戳停在 8-31 11:06,重新关账后一个字没变,内容还是「这个月净资产涨了220万…」。而页面上写着「**关账后结果缓存可回看**」,读起来就是本期定论。**修法两条**:① 重开时删掉该期缓存(放在重开那一刻,不等再次关账 —— 解读在数据被动的那一秒就作废了);② **进行中的期一律不碰缓存,既不读也不写** —— 写了更糟:这一期关账之后,那份月中生成的解读会被当成「本期定论」端出来。**教训**:凡是缓存,建的时候就要同时回答「谁来让它失效」;答不上来的缓存迟早会以「数字对、解读错、不报错」的形式出现,而这种错读起来完全通顺。 |
| v11916-SILENT-BALANCE-REWRITE | **改一笔收入会静默改掉已校准的余额**(2026-09-06 · 复现 issue #17 时顺带坐实)。删一笔 1,000 + 加一笔 4,321,已关账期的期末余额合计跟着变了(差额与那两笔改动的净额分毫不差) —— 页面没有任何提示。提交者把这描述成「已校准的余额失效」,**实际比这更重:不是失效,是被改了**。他因此把上个月账期重新打开重填了一遍。另外顺带查清了他困惑的第二个来源:**本月还没填余额的账户,它的变动不进 ΔNW** —— 复现时删掉的那 1,000 在「招行储蓄卡-工资」上,该账户当月未填余额(19 个账户填了 18 个),于是这 1,000 对仪表盘毫无影响。「改了数据但数字不动」因此**确实可能发生**,而页面没解释这个条件。两条都进 v1.20。 |
| KNOWN-PIVOT-VS-KPI-ROUNDING | **两条独立聚合路径的总资产差 0.01 · 与数据分布有关 · 尚未定位**(2026-09-06 记录)。e2e 断言「超级 Agent · AI 看到的总资产对不上」比对 `pivot`(PivotEngine)与 `period_summary`(FactViewService)两条**代码上无共用求和逻辑**的路径。当天早些时候还是绿的(175 全绿),在 beta 上做完 issue #17 的复现操作之后变成 `pivot=5555728.65 kpi=5555728.64`。把复现改动的金额还原后**数字一字未变**,说明它不是被那笔钱直接推动的,而是**某个账户的余额落点变化**改变了舍入路径。历史上这条也红过(曾见 0.03 / 0.06),**说明它是潜伏的、跟数据分布走的**,不是本次改动引入(v1.19.16 只碰 AI 复盘缓存)。**刻意不去还原数据把它压绿** —— 那等于把一个真实的口径不一致藏起来。留作 v1.20 的待查项:两条路径的四舍五入时机/精度需要对齐到一处。 |
| v1200-DIMENSION-THAT-DIDNT-EXIST | **照 TDD 去改一个不存在的东西**(2026-09-07 · v1.20 编码阶段)。TDD 说要在「归因/收益/透视/分布」四条路径上「把账户按组折叠」。打开代码才发现:`LensRegistry.DIMENSIONS` 只有 10 个属性维度,**根本没有「账户」这一维**;`PivotEngine` 里的 `byAccount` 去重只是为了账户级度量不重复计算。所以透视那两条不是「折叠账户」,而应该**新增一个「账户组」维度**(`Position.group`)。**教训**:TDD 的「前置检查」一节必须把要改的东西**打开看一眼**,不能凭对系统的印象写 —— 这一版前置检查抓到了两处(事实层不带划转对手方、账户在归因里是特例),但漏了这一处,因为我没去读 `LensRegistry`。 |
| v1200-GUARDRAIL-ON-A-COMMENT | **护栏 grep 的是注释,而 codeonly 会把注释剥掉**(2026-09-07 · v1.20)。第一版 `v1200-NET-NOT-GROSS` 判据是「解析器文档里要有『不需要对手方识别』这句话」—— 它守的是**一句话**,不是**行为**,写完当场就红(codeonly 剥注释)。改成钉在行为上:`AttributionEngine.groupBy` 必须是 `acc.merge(key, s.pnlBase(), BigDecimal::add)` 的成员求和,且解析器里不许出现求和调用。这条同时保证两件事:组内流转自动抵消(求和的副产品),以及不产生第二条求和路径。**教训**:判据里凡是 grep 中文说明的,基本都是在守一句话。 |
| v1200-A-NEW-FLASH-STARVES-THE-OLD-ONES | **新加一个 flash,饿死了四条与本版无关的断言**(2026-09-07 · v1.20)。改动提示走 `RedirectAttributes`。加上之后 e2e 四条支出断言变红,现象是「没落库、但也没有错误提示」。根因:e2e 的 `POSTcode` **不跟随 302**,前面步骤留下的 FlashMap 一直排在 session 队列里,后续 `GET /entry?period=N` 消费的是**队首那条陈旧的**,本次的 `flashError` 永远看不到 —— v1.20 之前这个队列基本是空的,所以从没暴露。**产品侧是对的**(无头浏览器实测:录收入→提示显示→刷新即消费→非法支出→错误正常弹出),修的是夹具:提交前把待消费的 flash **排空到干净**(还原「用户提交时已经在这一页上」的真实时序;上限给 5 不够,第一次调用时积压最多)。**顺带**:那条断言原来只会说「accepted」,把「真的被接受」和「页面根本没渲染出来」混成一句话;现在分别说 `no-flash-but-not-persisted` / `page-broken(len=…)` —— 正是靠这个分辨出「拒绝逻辑没坏,是提示丢了」。**教训**:断言的失败措辞要能区分「被守的东西坏了」和「观测手段坏了」。 |
| v1200-DELETE-MUST-BE-A-REAL-UNDO | **不可逆的「可逆」是陷阱**(2026-09-07 · v1.20)。`period_account_group` 刻意没有指向 `account_group` 的外键(它是历史快照)。第一版删组不动定格行 —— 结果历史里永远留着一个已经不存在的组名,而用户**没有任何办法退回按账户看**。而删组是这一版唯一的退路(不设开关,「有没有建组」本身就是开关)。修法:删组时连它的全部历史定格一起清。与「改成员不动历史」分得很清楚:**改成员 = 调整(历史不动)· 删组 = 撤销这件事本身(历史里它也该消失)**。**教训**:声称「可逆」之前,先走一遍那条退路,看它是不是真的退到了原点。 |
| v1200-SPLIT-ROUNDING-RESIDUAL | **穿透拆分逐份四舍五入,让两条聚合路径的总资产对不上**(2026-09-07 · v1.20 顺手修)。权重本身精确(`weight_bp` 合计恒为 10000),但把一笔市值拆成 9~10 份、每份各自 `setScale(2)` 之后求和会与原值差几分;而透视按份额加、KPI 按账户原值加 —— 两条路径因此对不上,且差多少**跟数据分布走**(有没有拆、拆几份、余数落在哪),历史上时红时绿,很容易被当成偶发。修法:**余数归位** —— 把「原值 − 各份之和」的残差补给份额最大的那一份(相对误差最小、也最不显眼);任一份不可算(null)时整列不动,**不能把 null 当 0 补,那是编数据**。**诚实记一笔**:这条修的是一个真实类别,但**它不是** e2e 那条「pivot 与 period_summary 差 0.01」的成因 —— 修完数字一分没变(当前 beta 数据的残差本来就是 0)。那条仍红着,仍是潜伏项,**没有为了让它变绿去动数据或放宽判据**。 |
| v1200-BUILT-IT-WHERE-NOBODY-LOOKS | **功能做完了,但只有翻到管理页才找得到**(2026-09-08 · v1.20 验收)。账户分组第一版做成了独立的 `/admin/account-groups`,而**账户列表与详情页一个字都没提**。维护者验收原话:「账户分组为什么还需要独立的管理页面,是否应该和账户管理想办法融合」「在账户列表详情页,应该增加去『管理账户分组』的入口」。**判断错在哪**:我把分组当成了「一项配置」,而它其实是**看账户的另一个方向** ——「从账户看它属于哪个组」和「从组看它含哪些账户」是同一件事的两侧。放在管理页等于要求用户先想到「这是配置」才找得到,而他是在看账户时想起来的。修法:搬到 `/accounts/groups`,列表加「分组」列、详情页加分组 pill、管理页保留一张卡片指过去。**教训**:新能力的入口要放在**用户想起它的那个场景**里,不是放在它「属于」的那个分类里。 |
| v1200-SILENT-STEAL-WORSE-THAN-ERROR | **账户被两个组选中时不报错,而是悄悄从原组搬走**(2026-09-08 · v1.20 验收)。第一版 `addMember` 用 `ON DUPLICATE KEY UPDATE group_id` 实现,那是**搬家语义**:在新组里勾一个已属别组的账户,系统把它从原组挪走,**页面一个字都不说**。原组少了一个成员、它的收益口径当场就变了,而用户不知道。**静默地改掉用户没打算改的东西,比报错更糟。** 修法三层:① 界面上这类账户**置灰 + 禁选 + 标出「已在『X』」**;② 后端 `requireAccountsFree` 直接拒绝并说清怎么挪(先去原组取消勾选);③ 单测钉住「拒绝时一行都不能写」(`verify(groups, never()).addMember(...)`)。**顺带纠正**:维护者报的「sql 错误」其实是另一件事 —— **组名重复**(`Duplicate entry '1-组A' for key 'uk_group_family_name'`)一路冒成 500 白页。两个缺陷被混成一条,复现时才分开。 |
| v1200-N-GROUPS-TIMES-ALL-ACCOUNTS | **每个分组都平铺全部账户的 checkbox**(2026-09-08 · v1.20 验收 · 维护者原话「分组管理页的交互怎么这么奇怪」)。19 个账户 × N 个分组,一屏全是勾选框,既看不出「现在有哪些分组」,也没法找。**根因是我按「表单」而不是按「列表」设计的** —— 想着「编辑成员」就直接把编辑态铺开了,忘了用户进来第一件事是**看**不是**改**。修法:分组卡片**默认折叠**(组名 + 成员数 + 成员名摘要),点开才是编辑态;顶部可按**分组名或成员账户名**搜索(命中自动展开),另有全部展开/收起。**教训**:管理类页面的默认态应该是「概览」,编辑是二级动作。 |
| v1200-SHORTCUT-TOOK-THE-BEST-SEAT | **把「建的时候的快捷方式」做成了常驻区块**(2026-09-08 · v1.20 二次验收)。分组页第一版把「推荐分组」做成列表页上的卡片墙,占着最好的位置。维护者原话:「进入分组管理,就是一个分组的列表,没有推荐;分组列表页右上角有新建,新建是弹窗,才有现在的『推荐』里面的内容」。**错在哪**:用户进这一页十次有九次是来**看**已有分组的,推荐只在第十次有用。修法:主页面 = 列表 + 搜索;右上角「+ 新建分组」开 `<dialog>`,推荐收进弹窗,而且只**预填**名字与成员 —— **两者都还能改**(维护者:「分组名也要允许用户自己维护」),推荐不是把名字写死。**教训**:一个功能在页面上占多大位置,应该按「用户来这一页要做什么」排,不是按「这个功能有多聪明」排。 |
| v1200-FEATURE-WITH-NO-DOOR | **下钻做完了,但页面上没有任何入口**(2026-09-08 · v1.20 二次验收)。分组折叠后想看单个账户,后端早就支持 `?expand=组ID`,但界面上一个按钮都没有 —— 只能手改 URL。这和「把分组藏进管理页」是同一个毛病:**做了但没人找得到,等于没做**。修法:「赚得最多/亏得最多」里,维值是**分组**时挂一个「点开」(是账户时不挂 —— 它已经是最细粒度,点了没有下一层),展开态顶部给「已展开分组 · 收起」横幅。实现上一个坑:这两块是 `innerHTML` 重绘出来的,**事件必须委托绑在容器上**,直接绑在按钮上会在下一次重绘时全部丢掉。 |
| v1200-PHANTOM-CSS-CLASS | **模板用了 CSS 里根本不存在的类,所有自动化都是绿的**(2026-09-08 · v1.20 三次验收)。维护者:「『建这个分组』太不像一个按钮了」。查下去发现**不是样式没调好,是 `btn-primary` 这个类全项目从未定义过**(实际有的是 `btn-ink`/`btn-paper`/`btn-ghost`)。浏览器对不存在的 class **完全不吭声**,于是按钮静静渲染成一行纯文字 —— 编译过、单测过、qa-run 过,**只有人眼能发现**。同一个错还在 v1.19 的 `admin/ai-access.html` 里躺了一个版本。**教训**:这类「静默降级」的错必须机械化 —— 落 `v1200-NO-PHANTOM-BTN-CLASS`,扫模板里所有 `btn-*` 逐个回查 `style.css`。凡是「浏览器不报错、只是长得不对」的,都是护栏的候选。 |
| v1200-COMPONENT-AUDIT-BY-INTENT | **盘查组件影响面要按「用户下一步干什么」分类,不是按「有没有账户字段」**(2026-09-08 · v1.20)。维护者要求逐个盘查 dashboard + reports 的全部组件。第一遍我按「模板里有没有 `accountName`」筛,筛出 8 个,**结论是错的** —— 同样是列账户名,「最大变动账户」是给你**看**的,「哪些账户没填余额」是让你去**填**的。正确判据是三分:**看**(统计/归因/分布 → 跟分组)· **做**(操作指令 → 不跟,组不是能记账的东西)· **险**(对手方风险 → 不跟)。**「险」这类差点漏掉**:HHI = Σ(份额²),5 个各占 4% 的账户合成一组 → HHI 从 0.008 涨到 0.04 **翻 5 倍**,指标会说「你的集中度暴增」而用户**什么都没做**。存款保险 50 万上限按**银行**算,不按用户的分组习惯算。22 个组件里只有 4 个该改。 |
| v1200-GROUP-RATIO-MUST-BE-NULL | **组行的收益率宁可给「—」也不给近似值**(2026-09-08 · v1.20)。账户列表 11 个指标列合并成组行时不是一类东西:金额可加、Δ% 可派生、sparkline 可重算,但 **XIRR / 本位币年化 / 最大回撤 / 预实必须 null**。XIRR 是资金加权年化,不能加也不能加权平均;想拿「累计损益 ÷ 累计净投入」顶替更糟 —— 那是**另一个口径**,填进「收益率」列 = 口径悄悄换了而页面一个字不说(`feedback_metric_refactor_baseline` 那条教训的形状)。最大回撤数学上能在合并时序上重算,但成员**开/关户**会造出跳变 → **凭空出现的回撤**。**一个错的收益率比没有收益率危险得多:它看起来完全合理,没人会去质疑它。** 代价(看不到组内各账户收益率)靠「组行可展开」补回来。 |
| v1200-INVENTORY-SOURCE-TOO-NARROW | **组件清单只取长文目录会漏三类东西**(2026-09-08 · v1.20 复核)。维护者要求复核我的盘查结论。清单第一版取自两页的 `tocItems`,理由是那是页面结构的显式声明 —— **但目录只收「值得跳转的锚点」**,于是漏了:① 没有独立 `id` 的块(dashboard「资产配置」饼图藏在 `#dash-trend` 里)② **不是 section 的交互件**(dashboard 账户多选筛选器是个 `<details>` 表单,不需要锚点)③ 一个 section 内的多个块(`#sec-risk` 有 2 张图、`#sec-structure` 有 4 块)。**清单从 22 个修正到 35 个块**,其中②是唯一被翻出来的真实缺口(补 FR-486)。修法:清单来源取**三者并集** —— `tocItems` + 各 fragment 的 `<section id>` + **所有 `eyebrow`/`<h3>` 标题块**(第三条才捞出「第三表」「流动性分层」这些)。**教训**:「页面一个 section」≠「用户眼里一个组件」,而最容易漏的恰恰是那些不需要锚点的交互件。 |
| v1200-REVIEW-CONFIRMS-CRITERION | **复核推翻了清单,但没推翻任何一条判断**(2026-09-08 · v1.20)。补进来 13 个块之后,按「看/做/险」三分判据逐个归类,**归属都是清楚的、没有一个边界模糊**,28 个「不动」一条都没翻案。这是判据本身可靠的证据 —— 一个好判据的标志不是「第一次就列全」,而是**新样本进来时不需要为它加特例**。反过来看第一版那个坏判据(「模板里有没有 accountName」):它连「最大变动账户」和「哪些账户没填余额」都分不开,加多少样本都救不回来。复核时对每个「不动」都回到代码找了数据源证据(`AllocationSlice.accountType` / `riskDistribution` / 模板里 accountId 出现 0 次 / `ConcentrationCalculator.line(…, thresholdPct)`),**不靠模板推断**。 |
| v1201-UNFALSIFIABLE-DIAGNOSIS | **一个「无法证伪」的诊断,让我把查证推给用户推了好几轮**(2026-09-08)。线上百炼智能体说「我这边没有数据查询工具」。我给的结论一直是「大概是 MCP 服务不在部署成功状态,你去控制台看看」—— 而这个结论**我自己无法验证**,百炼没有查询 MCP 服务状态的公开 API。于是每一轮都是:用户去看、回来说还不行、我再猜一个。**错在哪**:我把「我查不到」说成了「问题在那边」。真相其实一直躺在 `ask_access_audit` 里 —— **来过但 result=INVALID(口令过期/贴错)** 与 **一条记录都没有(它根本没来)** 是两个完全不同的病因、修法也完全不同,而它们在页面上长得一模一样。修法不是继续猜,是**把事实摆到页面上**:管理页显示最近 20 条入站访问(时间/来源/口令/调用/结果/UA),并区分本机与外部。**教训**:当一个诊断只能靠「请你去别处确认」来收尾时,它就不是诊断 —— 先去把能观测的那一面补上。 |
| v1201-CONFIG-FIELD-THAT-DOES-NOTHING | **「本站公网地址」这个字段只参与校验,不参与生成**(2026-09-08)。给百炼粘贴的 MCP 配置里,url 取的是 `guessBaseUrl(req)` —— **你打开管理页时用的那个域名**。于是在测试机上打开管理页,生成出来的是 `http://<内网IP>:20000/mcp`,百炼连不上;而页面上那个「本站公网地址」填得再对也没用。用户完全有理由以为「我明明填了公网地址」。而且线上实际填成了 `https://xxx/mcp`(把 `/mcp` 也带上了)—— 字段名叫「本站公网地址」、旁边示例又是一串 MCP 配置,填错太容易。修法:生成时**优先用用户填的那个地址**,请求头只作兜底;并在保存时把末尾的 `/mcp` 收掉。**教训**:一个让用户填了、却不参与实际行为的输入框,比没有这个框更糟 —— 它会让人停止怀疑真正的原因。 |
| v1202-BAILIAN-MCP-STOPPED-SERVING | **百炼的 MCP 服务「引用有效但不提供工具」,而且没有任何 API 能查它的状态**(2026-09-08 · 实测证据链)。线上 agent 一直说「我只有 mark_artifacts,查不到你家资产」。查证顺序:① 我们的 `/mcp` 本机与**公网 HTTPS** 都跑通 initialize/tools\_list/tools\_call ② prod 的 `/mcp` 公网可达(那个 404 是我们刻意返的「口令无效」,带 Spring Security 响应头 + Content-Length:0;不存在的路径返 302,两者可区分)③ `GET /agents/{id}` 确认 `system` 2684 字 + `mcp_servers` 引用都在 ④ **新建一个全新 agent 引用同一个 MCP,同样只有 mark_artifacts** → 排除老 agent 状态陈旧 ⑤ 会话期间 prod 审计表 max id 一直不变 → 百炼**一次都没来** ⑥ 但审计表里有 **09-03 的 13 条 `Bailian-MCP` 全 OK**(只有 initialize/notifications/tools\_list,**从来没有 tools\_call**)→ 它曾经好过。**结论**:MCP 服务在百炼侧已停止对外提供工具,只能在控制台「停止部署 → 重新部署」。**顺带记两条 API 事实**:创建 agent 时百炼**不会**去连我们的端点(引用校验只查它自己的注册表);agent 只能 `POST /agents/{id}/archive` 归档,`DELETE` 返回 405。 |
| v1202-UNFALSIFIABLE-CONCLUSION-NEEDS-A-TIMESTAMP | **无从下手的诊断,加一个时间戳就变成线索**(2026-09-08)。`testMcpLink()` 原来的失败提示是「大概是那个 MCP 服务不在部署成功状态,去控制台看看」—— 方向其实是对的,但它**不可证伪**(百炼没有查询 MCP 服务状态的公开 API),于是每轮都是「你去看 → 还不行 → 我再猜一个」,维护者原话「我说了非常多次要在 beta 跑通,你跑通了吗」。修法不是换个说法,是**去审计表里把「上次成功访问是什么时候」捞出来**:「上次成功是 09-03 15:24 —— 它曾经是好的,之后停了」立刻能让人对上「我那天改了什么」;而「历史上从来没成功过」指向的是完全另一种修法(压根没配对)。**教训**:当一个结论只能靠「请你去别处确认」收尾时,先问自己「我能不能至少说出它什么时候坏的」。 |
| v1202-MCP-SERVERS-DOES-NOT-MOUNT-TOOLS | **`mcp_servers` 只声明服务器,不挂工具 —— 挂工具要另外给 `tools/mcp_toolkit`**(2026-09-08 · 卡了好几天的真因)。症状:百炼智能体说「我只有 mark_artifacts,查不到你家资产」。**这个失败的每一步都是成功的**:① MCP 服务在控制台部署成功 ② 部署校验时百炼**真的连到了**我们的 `/mcp`(入站审计里 `initialize` + `tools/list` 全 OK)③ 创建 agent 返 200,回读 `mcp_servers` 引用也在 ④ 而 agent 在真实对话里一次 `tools/call` 都不发。于是排查方向被带到「MCP 服务是不是没部署成功」,**而它明明部署成功了**。正确形状是百炼逐条报错逼出来的:`tools[0].type` 合法值 `[builtin_toolkit, mcp_toolkit]` → `mcp_toolkit 必须指定 mcpServerName`(字段名用 snake `mcp_server_name`)→ `mcp_toolkit 引用的 mcpServerName 不在 mcpServers 列表中`(**两者必须同时给**)。**最反直觉的一条**:`default_config:{"enabled":true}` **一个人不够**,必须在 `configs` 里把每个工具名**逐个列出来** —— 名字叫 `default_config` 却不是「默认全开」。验证:事件流出现 `mcp_call`/`mcp_call_output`,审计表出现 `Bailian-MCP` 的 `tools/call`。 |
| v1202-VERIFY-MUST-CHECK-WHAT-MATTERS | **回读回读了,但没读那个真正会出错的字段**(2026-09-08)。v1.19.13 加过 `verifyTemplate()`,教训是「上游收下了不等于存住了」。但它只查 `system` 和 `mcp_servers` —— 而这次坏的是 `tools`,于是**空壳 agent 被判成创建成功**,页面显示「Agent 已创建」。回读的价值不在于「做了回读」,而在于**读的是不是那个会静默出错的字段**。现在 `hasPrompt && hasMcp && hasToolkit` 三者都查,并且失败提示会点名「MCP 服务引用在、但工具没挂上 —— 这个状态下 agent 会说自己只有『标记交付物』一个工具,而每一步都不报错」。 |
