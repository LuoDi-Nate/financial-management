...
public class ManagedAgentRuntime implements AgentRuntime {
    ...
    public String testMcpLink() {
        ...
        /* v1.20.2 · 把「上次什么时候还好着」一起说出来。
         *
         * 这条提示的上一版只说「大概是服务没部署成功,去控制台看看」——
         * 而那是一个【用户无从下手、我们也无法证伪】的结论:百炼没有查询 MCP 服务状态的公开 API。
         * 于是每一轮都变成「你去看 → 还不行 → 再猜一个」。
         *
         * 真正能让人动手的是【什么时候坏的】:一旦知道「上次成功是 09-03 15:24」,
         * 用户立刻能对上「我那天改了什么」。所以这里去审计表里把它捞出来。 */
        LocalDateTime lastOk = auditMapper.lastBailianOkAt(FAMILY_ID);
        String when = lastOk == null
                ? "而且历史上从来没有成功过一次 —— 说明这个 MCP 服务【一次都没真正跑起来】。"
                : "而上一次它成功访问是 " + lastOk.format(java.time.format.DateTimeFormatter.ofPattern("MM-dd HH:mm"))
                  + " —— 也就是说它【曾经是好的,之后停了】。想想那之后你在百炼上改过什么。";
        return "不通:这一问期间百炼一次都没有访问你的账房。" + when
             + " 会话建起来了、答案也回来了,所以不是凭据、地址或 Agent 的问题;"
             + "MCP 引用也是对的 —— 名字填错百炼会直接拒绝创建 Agent。"
             + "去百炼「MCP 管理 → 自定义服务」看这个服务的状态;改过配置的话必须"
             + "「停止部署 → 重新部署」才生效。不要去动服务 ID。"
             + "页面下面「谁访问过这个账房」那张表可以自己核对这个判断。";
    }
