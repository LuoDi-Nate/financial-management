...
**[v1.20.2](https://github.com/LuoDi-Nate/financial-management/releases/tag/v1.20.2) · 百炼托管 Agent 修好:挂工具要 `tools/mcp_toolkit`,不只 `mcp_servers`**(并含 v1.20.1)
智能体一直说「我只有标记交付物一个工具、查不到资产」,而每一步都是成功的:MCP 服务部署成功、百炼真的连上过我们的端点、创建 Agent 返 200、回读引用也在。真因:`mcp_servers` 只**声明**服务器,挂工具必须同时给 `tools/mcp_toolkit` 且在 `configs` 里**逐个列出工具名**(`default_config:{enabled:true}` 一个人不够)。一并修:回读校验补查 `tools`(空壳 agent 不再被判成功)、自检说出**上次成功访问是什么时候**、Agent 名字带域名 + 更新前校验归属(防测试环境一键覆盖生产)、管理页新增「谁访问过这个账房」入站记录。**升级后在管理页点一次「创建 Agent」再自检。无 DB 迁移。**

**[v1.20.0](https://github.com/LuoDi-Nate/financial-management/releases/tag/v1.20.0) · 账户组:十几个活期账户,统计里并成一行**
有用户提:银行卡 / 微信 / 支付宝之间来回转账,归因和分布里全是**互相抵消的噪声**。现在可以把它们归成一组,统计里合成一行 —— 组内划转自动抵消,不需要识别对手方。**没建组的话页面一个像素都不变**,没有开关(「有没有建组」本身就是开关)。组行的**收益率 / 回撤这类比率显示「—」**:XIRR 是资金加权年化,不能相加也不能加权平均,拿另一个口径顶替等于悄悄换口径 —— 想看就点组名展开。这一版逐个盘查了两个核心页面的 **35 个组件块**,只有 7 个跟着分组走:「去填余额 / 从 X 转到 Y」这类操作入口、以及**集中度**都**故意不跟**(分组是你的看法,不改变钱实际存在哪家机构),并在页面上说明为什么。同一个 issue 顺带修了「改收入会悄悄改掉已校准余额却零提示」。**含 DB 迁移 V58**(三张全新表,不改既有表)。
