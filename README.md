...
**[v1.19.12](https://github.com/LuoDi-Nate/financial-management/releases/tag/v1.19.12) · 百炼接入教程漏了「模型授权」这一步**
按教程一步步配到最后,点「创建 Agent」收到「模型不存在: model=qwen-plus」—— 而模型是存在的,真正的原因是那是个**子业务空间,没有被授予模型调用权限**。问题不在这句报错,在于**教程从头到尾没有「模型」这一环**:你每一步都做对了,失败信息却指向一个你没做错的地方。最难自己想到的是「聊天能用不代表这里能用」(对话接口不带业务空间,托管 Agent 的地址嵌着它)。这一版把向导从 5 步扩到 6 步、模型改成可配(原来写死)、失败提示按上游原话分流并且认不出来的**明说自己在猜**。**无 DB 迁移**,已配好能用的不需要做任何事。
